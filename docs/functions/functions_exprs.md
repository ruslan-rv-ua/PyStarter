...
У вищенаведеному прикладі спочатку функція `len()` повертає кількість символів у рядку, 
а потім до цього значення додається число 4.
...
Спочатку обчислюється вираз `'a' + 'bc'`, результатом якого є рядок `'abc'`, 
потім цей рядок передається функції `len`, яка повертає кількість символів у рядку.
...
Спочатку функція `len` повертає кількість символів у рядку `'a'`,
потім це значення передається функції `print`, яка виводить його на екран.
