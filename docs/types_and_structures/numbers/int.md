...
При створенні за допомогою літерала можна вказати систему числення 
...
Важливою особливістю цілих чисел у Python є те, що вони мають довільну точність. Це означає, що вони можуть представляти числа будь-якого розміру, обмежені лише доступною пам'яттю системи.
...
Цілі числа, як і інші вбудовані типи в Python, мають ряд спеціальних методів. Ось деякі з них:

| Метод          | Оператор/Функція | Опис                                                                 |
|----------------|------------------|----------------------------------------------------------------------|
| `__lshift__(self, other)`| `<<`             | Бітовий зсув вліво                                                   |
| `__rshift__(self, other)`| `>>`             | Бітовий зсув вправо                                                  |
| `__and__(self, other)` | `&`              | Бітове "І" (AND)                                                     |
| `__or__(self, other)`  | `|`              | Бітове "АБО" (OR)                                                    |
| `__xor__(self, other)` | `^`              | Бітове "виключне АБО" (XOR)                                          |
| `__invert__(self)`   | `~`              | Бітова інверсія (NOT)                                                |
| `__int__(self)`      | `int()`          | Перетворення на ціле число (зазвичай повертає `self`)                 |
| `__index__(self)`    |                  | Використовується для перетворення на цілочисельний індекс (напр. у зрізах) |
