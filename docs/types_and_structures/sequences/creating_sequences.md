...
У Python зрізи представлені вбудованим класом `slice`. 
...
`slice` має метод `indices()`, який дозволяє отримати конкретні індекси, які будуть використані при застосуванні цього зріза для послідовності заданої довжини. 

    slice_object.indices(sequence_length)

де `sequence_length` — довжина послідовності, до якої застосовується зріз.
