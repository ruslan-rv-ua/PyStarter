...
> ***Словник*** (dictionary, асоціативний масив) — мутабельна колекція пар "ключ-значення", яка, **починаючи з версії Python 3.7, зберігає порядок вставки елементів**, та підтримує довільний доступ до її елементів (значень) по відповідних ключах.
...
|`d.popitem()`|Видаляє **останню додану (LIFO)** пару ключ-значення і повертає її (У версіях Python до 3.7 видалялася довільна пара). Якщо словник порожній — піднімає виняток `KeyError`|
