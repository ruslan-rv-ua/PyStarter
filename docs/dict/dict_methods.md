...
Видаляє та повертає пару у вигляді кортежа (ключ, значення) зі словника "d".
У Python 3.7+ метод повертає останню додану пару.
У попередніх версіях Python видаляв і повертав випадкову пару.
Якщо словник порожній, спричиняє вийняткову ситуацію 'KeyError'.
...
Повертає значення за ключем "key". Якщо ключа немає у словнику, додає його зі значенням "default" (за замовчуванням None) і повертає це значення.
	...
	>>> d
	{1: 11, 2: 22, 3: 'foo'}
	>>> d.setdefault(3, 'bar')  # значення не змінюється, якщо ключ вже існує
	'foo'
	>>> d
	{1: 11, 2: 22, 3: 'foo'}
	>>>
...
Створює новий словник з ключами з послідовності "seq"
і значенням "value" (за замовчуванням None).
...
Зауважте що метод `fromkeys()` викликається на класі `dict`, а не на об'єкті словника.
