...
<!-- 1. Після запуска у графічному інтерфейсі операційної системи відкривається нове вікно термінала для взаємодії з користувачем. -->
1. Інтерпретатор читає з вказаного файла інструкції, рядок за рядком, і виконує їх.
<!-- 1. Після завершення виконання початкового коду вікно термінала закривається, інтерпретатор завершує роботу. -->

### Запуск Python-програм

Програми на Python зберігаються у текстових файлах які мають розширення (тип) **.py**.

Щоб запустити програмний код на виконання у терміналі виконаємо команду **python** вказавши їй **.py**-файл з програмним кодом, наприклад:

	python my_app.py

Тобто буде виконано програмний код який міститься у файлі "my_app.py". У даному прикладі файл очікується у поточній папці. Можна також вказати абсолютний шлях до файла, який треба виконати, наприклад:
...
- Меню "Плагіни - PyNPP - Run file in Python", або ж комбінація клавіш **Ctrl + Alt + F5**. Відкривається термінал, після закінчення роботи програми термінал закривається автоматично.
- Меню "Плагіни - PyNPP - Run file in Python interactive", або ж комбінація клавіш **Alt + Shift + F5**. Відкривається термінал, після закінчення роботи програми консоль не закривається, інтерпретатор переходить в інтерактивний режим.
- Меню "Плагіни - PyNPP - Run file in PythonW", або ж комбінація клавіш **Ctrl + Alt + Shift + F5**. Запуск без відкриття термінала Windows.
...
коли код було записано з порушенням граматичних правил. 
...
Аналізуючи код знаходимо помилку: для операції присвоєння не вказано значення яке треба присвоїти. 
