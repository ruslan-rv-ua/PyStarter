...
(так само як операція множення для чисел) і називається *"реплікація"*, або ж (рідко) — *"мультиплікація"*. 
...
У реплікації пріорітет вище ніж у конкатенації:
...
Зауважте: вирази, які містять операції конкатенація і реплікація в результаті призводять до значення типу даних "символьний рядок".
