...
Перший параметр "file_path" — це шлях до файлу у файловій системі.
...
- r (Read). Файл відкривається для читання. Якщо файлу не існує, "піднімається" вийняток FileNotFoundError. Це режим за замовчуванням.
...
- x (eXclusive). Файл відкривається для запису. Якщо файл вже існує, "піднімається" вийняток FileExistsError.

Існує ще декілька режимів, їх буде розглянуто у наступних темах.

Функції *open()* можна вказати у якому кодуванні файл, з яким ми хочемо працювати. Зробити це можна наступним чином: після вказання імені файла і режиму у якому він відкривається ми вказуємо ще один аргумент. Записується це так:

> encoding=<кодування>

*<кодування>* — це об'єкт типу *str*, у якому у символьному вигляді представлена назва кодування текста. Дуже зручно!
...
Надалі з файлом взаємодіють через методи цього об'єкта.
