...
В інтрерпретаторі Python є спеціальний механізм — збирач сміття (garbage collector). 
Спрощено це працює так: у певні моменти часу збирач сміття "пробігається" по усім об'єктам в пам'яті, 
...
Наприклад, якщо видаляється список, то лічильник посилань у всіх його элементах зменшується на 1. 
Якщо об'єкти всередині списка більше ніде не використовуються, то їх також будеь видалено. 
...
Змінні, котрі оголошено всередині функції, є локальними. 
Як тільки інтерпретатор виходить з функції він знищує усі посилання створені локальними змінними всередині неї. 
